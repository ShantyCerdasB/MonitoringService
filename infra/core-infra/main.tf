# Main infrastructure configuration for the monitoring service
# Creates and orchestrates all Azure resources required for the application

# Primary resource group for all infrastructure components
...
# Static Web App for the admin interface
...
module "aad_spa" {
  ...
  aad_redirect_uris             = concat(var.aad_redirect_uris)
...
module "function_app" {
  ...
  depends_on = [module.keyvault, module.aad_spa]
}
...
# PostgreSQL database (currently disabled)
module "postgres" {
...
module "keyvault" {
  ...
  depends_on = [module.aad_spa, module.web_pubsub]
}